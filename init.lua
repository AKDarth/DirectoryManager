...
local function ReturnCompatability(Table, IteratorFlag) -- ReturnCompatability (Table: table, IteratorFlag: boolean)
	Table = ( type(Table) == 'table' and Table ) or Error.With(): InvalidArgument():At {
		Line = 41, Function = 'ReturnCompatability', ValueName = tostring(Table),
	};
	local _ = ( type(IteratorFlag) == 'boolean' ) or Error.With(): InvalidArgument():At {
		Line = 42, Function = 'ReturnCompatability', ValueName = tostring(IteratorFlag),
	};
...
local function SerializePathArguments(Table) -- SerializePathArguments (Table: table)
	Table = ( ReturnCompatability(Table, false) == 'ipairs' and Table ) or Error.With(): InvalidArgument():At {
		Line = 52, Function = 'SerializePathArguments', ValueName = ReturnCompatability(Table, false),
	};
	Table = ( #Table ~= 0 and Table) or Error.With(): InvalidArgument():At {
		Line = 53, Function = 'SerializePathArguments', ValueName = tostring(Table),
	};
...
function DirectoryManager.PathSearchAsync(Information) -- .PathSearchAsync (Information: table)
	Information = ( type(Information) == 'string' and Information ) or Error.With(): InvalidArgument():At {
		Line = 95, Function = '.PathSearchAsync', ValueName = type(Information),
	};
...
function DirectoryManager.Init(RequestedEnvironment) -- .Init (RequestedEnvironment: string)
	RequestedEnvironment = ( type(RequestedEnvironment) == 'string' and RequestedEnvironment ) or Error.With(): InvalidArgument():At {
		Line = 103, Function = '.Init', ValueName = type(RequestedEnvironment),
	};
	
	RequestedEnvironment = Environments[RequestedEnvironment] or Error.With(): InvalidArgument():At {
		Line = 107, Function = '.Init', ValueName = RequestedEnvironment,
	};
...
function DirectoryManager.SafeLoadComponent(Component) -- .SafeLoadComponent (Component: userdata)
	Component = ( type(Component) == 'userdata' and Component ) or Error.With(): InvalidArgument():At {
		Line = 120, Function = '.SafeLoadComponent', ValueName = type(Component),
	};
end;
